...
def evaluate_tf_laurent(f, poles, residues, const, diff, zero=None):
...
def is_controllable(A, B):
    n = A.shape[0]
    C = np.hstack([np.linalg.matrix_power(A, i).dot(B) for i in range(n)])
    return matrix_rank(C) == n


def is_observable(A, C):
    n = A.shape[0]
    O = np.vstack([C.dot(np.linalg.matrix_power(A, i)) for i in range(n)])
    return matrix_rank(O) == n


def balanced_realization(A, B, C):

    """
    Perform balanced realization to obtain 
    numerically robust state-space matrices.
    
    INPUTS:
        A, B, C : State-space matrices from original realization
    
    """
    
    # Solve continuous-time Lyapunov equations to get Gramians P and Q
    P = solve_continuous_lyapunov(A, np.dot(B, B.T))
    Q = solve_continuous_lyapunov(A.T, np.dot(C.T, C))
    
    # Cholesky factorization of Gramians P and Q
    L = np.linalg.cholesky(P)
    M = np.linalg.cholesky(Q)
    
    # Singular Value Decomposition of L^T * M
    U, Sigma, Vt = svd(np.dot(L.T, M))
    
    # Compute transformation matrices T and T_inv
    T = np.dot(np.dot(np.linalg.inv(L), U), np.sqrt(Sigma))
    T_inv = np.dot(np.sqrt(Sigma), np.dot(Vt, np.linalg.inv(M)))
    
    # Compute balanced realization matrices
    Ab = np.dot(np.dot(T_inv, A), T)
    Bb = np.dot(T_inv, B)
    Cb = np.dot(C, T)
...
def gilbert_realization(Poles, Residues, Const, Diff):
    ...
    INPUTS : 
        ...
        Diff     : (array) matrix for differentiating term
